...
export default function DeleteClass() {
  ...
  const deleteRecord = (event) => {
    ...
    AxiosInstance.delete(`classes/${id}/`)
      .then(() => {
        setIsError(false);
        alert("Class deleted successfully. Redirect to the list");
      ...
      <form onSubmit={deleteRecord}>
        <Box className="formBox" sx={{display: "flex", flexDirection: "column"}} onSubmit={deleteRecord}>
          <Box className="formArea">
            <Typography>
              You will delete class <strong>{currentClass.name}</strong>{" "} of lecturer{" "}
              <strong>{currentClass.lecturer_name}</strong>.
            </Typography>
          </Box>
